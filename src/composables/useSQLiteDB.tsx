...
const useSQLiteDB = () => {
  ...
  const initializeTables = async () => {
    ...
    const createClassTable = `CREATE TABLE IF NOT EXISTS class (class_id INTEGER PRIMARY KEY NOT NULL, module_id INTEGER, specialty_id INTEGER, scholar_year_id INTEGER, FOREIGN KEY (module_id) REFERENCES module(module_id) ON DELETE  NO ACTION, FOREIGN KEY (specialty_id) REFERENCES specialty(specialty_id) ON DELETE  NO ACTION, FOREIGN KEY (scholar_year_id) REFERENCES scholar_year(scholar_year_id) ON DELETE NO ACTION);`
    ...
    await performSQLAction(async (db: SQLiteDBConnection | undefined) => {
      ...
      if (respSELECTED_YEAR?.values && respSELECTED_YEAR?.values?.length == 0) {
        ...
        const respLATES_YEAR_ID = await db?.query(`SELECT scholar_year_id FROM scholar_year WHERE year IN (SELECT MAX(year) FROM scholar_year );`)

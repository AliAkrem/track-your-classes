...
export default function CreateGroupModal({
  isOpen,
  setToClose,
  selectedClassIDToAddGroup,
  setSelectedClassIDToAddGroup,
}: Props) {
  // const [modalCreateGroupOpened, setModalCreateGroupOpened] = useState(false)

  const { setRevalidate } = useGlobalContext();

  const create_class_modal_create_group = useRef<HTMLIonModalElement | null>(
    null
  );
  ...
  const addStudent = useRef<HTMLIonInputElement>(null);
  ...
  const CREATE_GROUP = async (
    ...
    try {
      await performSQLAction(async (db: SQLiteDBConnection | undefined) => {
        await db?.query(
          `INSERT INTO Groupp( group_type, group_number, class_id) VALUES ( ?, ?, ?)`,
          [group_type, group_number, selectedClassIDToAddGroup]
        );

        const resGroup_id: any = await db?.query(
          `SELECT group_id as id FROM Groupp WHERE group_id IN (SELECT max(group_id) FROM Groupp) AND class_id = ? ;`,
          [selectedClassIDToAddGroup]
        );
        ...
        await Promise.all(
          student_list.map(async (student) => {
            await db?.query(
              `INSERT OR IGNORE INTO student( first_name, last_name, student_code) VALUES (?, ?, ?)`,
              [student.first_name, student.last_name, student.student_code]
            );
            const resStudent_id: any = await db?.query(
              `SELECT max(student_id) as id FROM student WHERE  student_code = ? `,
              [student.student_code]
            );
            const student_id = resStudent_id?.values[0]?.id;
            await db?.query(
              `INSERT OR IGNORE INTO group_student( group_id, student_id) VALUES (?, ?)`,
              [group_id, student_id]
            );
          })
        );
  ...
  const onCreateGroup = async () => {
    if (
      type_of_group.current?.value !== "" &&
      group_number.current?.value !== "" &&
    ...
    ? student_list?.map((student, idx) => (
        <IonItem key={nanoid()}>
          ...
          <IonLabel class="ion-text-wrap">
            <h2>
              {idx + 1} - {student.first_name.toLowerCase()}{" "}
  ...
  return (
    <IonModal
      ref={create_class_modal_create_group}
      isOpen={isOpen}
      onIonModalDidDismiss={() => {
        create_class_modal_create_group?.current?.dismiss();
        setSelectedClassIDToAddGroup(-1);
        setToClose(false);
      }}
    >
      ...
      <IonContent fullscreen>
        <IonItem className="Ion-padding">
          <DragDropFile
            ...
            setStudent_list={setStudent_list}
          />
        </IonItem>
          ...
          <IonButton
            onClick={() => {
              if (
                ...
                String(addStudent.current?.value).length > 3
              ) {
                setStudent_list([
                  ...student_list,

                  {
                    ...
                    student_code: (student_list.length + 1).toString(),
                    first_name:
                      String(addStudent.current?.value).split(" ")[0] ?? "",
                    last_name:
                      String(addStudent.current?.value).split(
                        String(addStudent.current?.value).split(" ")[0] + " "
                      )[1] ?? "",
                  } as any,
                ]);
                if (addStudent.current) addStudent.current.value = null;
              }
            }}
          >
